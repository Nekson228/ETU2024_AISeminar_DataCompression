...
**Замечание:** Проецированные вектора в исходном прострнастве $\tilde{x}$, которые получались естественным образом в PCA, в KPCA не так просто получить. Однако, их получение не имеет большого смысла, поскольку KPCA используется для построения нелинейных моделей, а не для визуализации данных.
